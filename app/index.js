/**
 * 数字娱乐业务线强制建立src目录，src下有lib|mock|page|widget|util
 * lib存放第三方库
 * mock存放假接口数据
 * page存放demo页面
 * widget存放与业务关联的模块
...
var ClamGenerator = module.exports = function ClamGenerator(args, options, config) {
    ...
    this.on('end', function () {
  		...
  		this.prompt([{
  				...
  				if(this.npm_install){
  					this.spawnCommand('tnpm', ['install'], function (err) {

  						if (err) {
  							return console.log('\n'+yellow('please run "sudo npm install\n npm install -g tnpm --registry=http://registry.npm.alibaba-inc.com"\n'));
  						}

  						console.log(green('\n\ntnpm was installed successful. \n\n'));
  					});
  				} else {
  					console.log(yellow('\n\nplease run "tnpm install" before grunt\n'));
  					console.log(green('\ndone!\n'));
  				}
  		}.bind(this));
    }.bind(this));
